// ============================================================
        ...
        function initSkyCanvas() {
            ...
            window.addEventListener('resize', function() {
                ...
                skyResizeTimer = setTimeout(resize, 120);
            }, { passive: true });

            var isSmallScreen = window.matchMedia && window.matchMedia('(max-width: 768px)').matches;
            var reduceMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
            var starCount = reduceMotion ? 220 : (isSmallScreen ? 650 : 1800);
            var stars = [];
            for (var i = 0; i < starCount; i++) {
                var mag = Math.pow(Math.random(), 2.5);
                var hue = Math.random() < 0.12 ? (Math.random() < 0.5 ? 220 : 30) : 0;
            ...
            function drawStars(w, h) {
                for (var i = 0; i < stars.length; i++) {
                    ...
                    if (s.bright > 0.80 && r > 1.2) {
                        ...
                        ctx.strokeStyle = s.hue !== 0 ? 'hsl(' + s.hue + ',' + s.sat + '%,85%)' : '#c8daff';
                        ctx.lineWidth = 0.5;
                        ctx.beginPath();
                    ...
                    if (r > 0.9) {
                        ...
                        var col = s.hue !== 0 ? 'hsla(' + s.hue + ',' + s.sat + '%,85%,' : 'rgba(210,225,255,';
                    ...
                    ctx.fillStyle = s.hue !== 0 ? 'hsla(' + s.hue + ',' + s.sat + '%,92%,' + alpha + ')' :
            ...
            function drawMoon(w, h) {
                var mx = moonX * w;
                var my = moonY * h;
                var mr = Math.min(moonR, w * 0.12, h * 0.15);
                ...
                var base = ctx.createRadialGradient(mx - mr * 0.28, my - mr * 0.28, mr * 0.05, mx, my, mr);
        ...
        function initAOS() {
            ...
            AOS.init({ duration: window.innerWidth <= 768 ? 550 : 800, easing: 'ease-in-out', once: true, offset: window.innerWidth <= 768 ? 45 : 100, disable: window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches });
        }
        ...
        function initGSAP() {
            ...
            gsap.from('.hero-image', { scale: 0, rotation: 180, opacity: 0, duration: 1.4, ease: 'back.out(1.7)',
                delay: 0.3 });
            gsap.from('.date-badge', { scale: 0, opacity: 0, duration: 0.8, ease: 'elastic.out(1,0.5)', delay: 1.1 });
            gsap.from('.counter-box', { y: 40, opacity: 0, duration: 0.7, stagger: 0.18, ease: 'power3.out',
                delay: 1.4 });
        ...
        // ============================================================
        var quizData = [
            { question: "What is Boo's full name?", options: ["Dewmini Savindi Katawala", "Dewmini Sanduni Katawala",
                    "Dewmini Sithumi Katawala", "Dewmini Senuri Katawala"
                ], correct: 0 },
            { question: "What is Boo's secret nickname?", options: ["Boo 🐰", "Boo 🐻", "Boo 🐱", "Boo 🦋"],
            correct: 1 },
            { question: "What is Boo's favorite sugary treat?", options: ["Chocolate", "Lollipops", "Cotton Candy",
                    "Gummy Bears"
                ], correct: 2 },
            { question: "What degree is Boo pursuing?", options: ["Computer Science", "Electrical & Telecommunication Engineering",
                    "Mechanical Engineering", "Civil Engineering"
                ], correct: 1 },
            { question: "What is Boo's favorite color?", options: ["Pink", "Purple", "Blue", "Green"], correct: 2 },
            { question: "When is Boo's birthday?", options: ["June 25, 2002", "July 25, 2002", "June 15, 2002",
                    "July 15, 2002"
                ], correct: 0 },
            { question: "Where is Boo studying?", options: ["University of Colombo", "HND Galle",
                    "University of Peradeniya", "University of Moratuwa"
                ], correct: 1 },
            { question: "What does Boo love to watch at night?", options: ["Movies", "The night sky", "TV Shows",
                    "Fireworks"
                ], correct: 1 }
        ];
        ...
        function initQuiz() { updateQuizProgress();
            showQuestion(); }
        ...
        function startGame() {
            ...
            if (btn) { btn.textContent = 'Game Running...';
                btn.disabled = true; }
            if (area) area.innerHTML = '';

            var spawnDelay = window.innerWidth <= 768 ? 760 : 600;
            starInterval = setInterval(function() { if (gameActive) spawnStar(); }, spawnDelay);
        ...
        function spawnStar() {
            ...
            var safeX = Math.max(0, (area.clientWidth || 320) - 48);
            star.style.left = Math.floor(Math.random() * safeX) + 'px';
            star.style.top = '-30px';
            ...
            function collectStar(e) {
                ...
                if (!gameActive || star.classList.contains('collected')) return;
                gameScore++;
                var scoreEl = document.getElementById('gameScore');
            ...
            star.addEventListener('pointerdown', collectStar, { passive: true });
        ...
        function endGame() {
            ...
            if (area) {
                ...
                area.innerHTML = '<div style="display:flex;flex-direction:column;justify-content:center;align-items:center;height:100%;color:#f5c842;padding:16px;">' +
                    '<div style="font-size:clamp(2.4rem,6vw,3rem);margin-bottom:12px;">🎉</div>' +
                    '<div style="font-size:clamp(1.6rem,4vw,2rem);font-weight:700;">Game Over!</div>' +
                    '<div style="font-size:clamp(1.2rem,3vw,1.4rem);margin-top:8px;">You caught ' + gameScore +
                    ' stars!</div>' +
                    '<div style="font-size:clamp(0.85rem,1.5vw,0.95rem);color:#8a9fb8;margin-top:8px;">' + msg +
                    '</div>' +
                    '</div>';
            }
        ...
        function isAdjacent(a, b) {
            var ar = Math.floor(a / PUZZLE_SIZE),
                ac = a % PUZZLE_SIZE;
            var br = Math.floor(b / PUZZLE_SIZE),
                bc = b % PUZZLE_SIZE;
        ...
        function showPuzzleWin() {
            ...
            banner.innerHTML =
                ...
                '<p style="color:#9ab0cc;font-size:clamp(0.8rem,1.2vw,0.9rem);margin-bottom:14px;">' +
        ...
        // ============================================================
        // VIDEO FALLBACK / MOBILE PLAYBACK
        // ============================================================
        function initVideo() {
        ...
        function initCounters() {
            ...
            if (ageEl) {
                var age = 0,
                    target = 24;
                var iv = setInterval(function() { ageEl.textContent = ++age; if (age >= target) clearInterval(iv); },
                90);
            }
            if (daysEl) {
                var days = 0,
        ...
        var quotes = [
            ...
            { text: "Some souls are just meant to be, even if the world tries to keep them apart.",
            author: "Unknown" },
        ...
        function initGallery() {
            var carouselWrap = document.querySelector('.carousel-container');
            var startX = 0;
            var startY = 0;
            ...
            if (carouselWrap) {
                ...
                carouselWrap.addEventListener('pointerup', function(e) {
                    ...
                    if (Math.abs(dx) > 36 && Math.abs(dx) > Math.abs(dy) * 1.25) {
                        gallerySwipeUntil = Date.now() + 420;
                        if (dx < 0) nextGallery();
                        else prevGallery();
                    }
                }, { passive: true });
            ...
            document.querySelectorAll('.carousel-card').forEach(function(item) {
                item.addEventListener('click', function(e) {
                    if (Date.now() < gallerySwipeUntil || Date.now() < gallerySuppressLightboxUntil) return;
        ...
        var gallerySuppressLightboxUntil = 0;
        var gallerySwipeUntil = 0;

        function getCenteredGalleryIndex() {
            var idx = Math.round((-galleryAngle) / galleryStep) % galleryTotal;
            return (idx + galleryTotal) % galleryTotal;
        }
        ...
        function updateGalleryCarousel() {
            ...
            var radius = isMobile ? Math.max(104, Math.min(142, vw * 0.34)) : 300;
            var cardW = isMobile ? Math.max(126, Math.min(172, vw * 0.40)) : 280;
            var cardH = isMobile ? Math.round(cardW * 1.34) : 360;
            ...
            cards.forEach(function(card, i) {
                ...
                var rotateY = -angle;
                card.style.transform = 'translateX(' + x + 'px) translateZ(' + z + 'px) rotateY(' + rotateY +
                    'deg)';
                card.style.opacity = z > -100 ? 1 : (isMobile ? 0.18 : 0.3);
        ...
        function rotateGallery(index) {
            var wasCentered = getCenteredGalleryIndex() === index;
            galleryAngle = -index * galleryStep;
            updateGalleryCarousel();
            if (window.innerWidth <= 768 && !wasCentered) {
                gallerySuppressLightboxUntil = Date.now() + 360;
            }
        }
        ...
        function initSparkleClick() {
            document.addEventListener('click', function(e) {
                ...
                if (tag !== 'button' && tag !== 'a' && !e.target.closest('.quiz-option') && !e.target.closest(
                        '.game-star') && !e.target.closest('.puzzle-tile')) {
        ...
        function initFooterHeart() {
            ...
            if (h) {
                h.addEventListener('click', function() {
                    ...
                    var self = this;
                    setTimeout(function() { self.style.animation = 'heartbeat 1.5s ease-in-out infinite'; },
                    2000);
                });
            }
        }
        ...
        function consoleMessage() {
            ...
            console.log('%cPress C for confetti, S for candy rain, Esc to close lightbox!',
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            try { initVideo(); } catch (e) { console.warn('Video error:', e.message); }
            ...
            try { initCarouselResize(); } catch (e) { console.warn('Carousel resize error:', e.message); }
            consoleMessage();
        ...
        // Re-run carousel after full load
        window.addEventListener('load', function() {
            setTimeout(updateGalleryCarousel, 300);
        });
